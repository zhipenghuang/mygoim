...
func WsEecode(pack Package) ([]byte, error) {
	var err error
	var WriteBuf = make([]byte, BufLen)
	contentLen := len(pack.Content)
	if contentLen > ContentMaxLen {
		return nil, ErrOutOfSize
	}

	binary.BigEndian.PutUint16(WriteBuf[0:TypeLen], uint16(pack.Code))
	binary.BigEndian.PutUint16(WriteBuf[LenLen:HeadLen], uint16(len(pack.Content)))
	copy(WriteBuf[HeadLen:], pack.Content[:contentLen])

	return WriteBuf[:HeadLen+contentLen], err
}
