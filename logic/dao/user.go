...
// GetByNumber 获取用户信息根据手机号
func (*userDao) All(ctx *imctx.Context) ([]*model.User, error) {
	rows, err := ctx.Session.Query("select id,number,nickname,password,sex,avatar from t_user")
	if err != nil {
		logger.Sugar.Error(err)
		return nil, err
	}
	...
	for rows.Next(){
		user := new(model.User)
		err := rows.Scan(&user.Id, &user.Number, &user.Nickname, &user.Password, &user.Sex, &user.Avatar)
		if err != nil {
			logger.Sugar.Error(err)
			return nil, err
		}
